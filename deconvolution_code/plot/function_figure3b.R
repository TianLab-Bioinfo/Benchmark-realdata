trend_plot <- function(ER_files,ER_dir,TNBC_dir,key_trend,disease){
  ...
  key_trend[key_trend$Trend=='-','value'] <- 0-key_trend[key_trend$Trend=='-','PValue']
  key_trend[key_trend$Trend=='+','value'] <- key_trend[key_trend$Trend=='+','PValue']
  key_trend[,'orig'] <- 'scRNA'
  key_trend=key_trend[,c('orig','Celltype','value')]
  colnames(key_trend) <- c('orig','variable','value')
  melt_df_wide <- rbind(melt_df_wide,key_trend)
